...
<div class="balance-container">
    <div class="balance-box">
        ...
        $row = $res->fetch_assoc();
         echo "<p style = 'background: linear-gradient(90deg, rgb(28, 28, 28) 75%, rgb(79, 78, 78)); padding:8px; border-radius:10px; color:white;'><i class='fas fa-money-bill-wave'></i> Bakiye: " . number_format($row['balance'], 2, ',', '.') . " TL</p>";
        ?>
    </div>
...
<div class="main-content">
    ...
    <?php
    // En son güncellenen kurun zamanını kontrol et
    $res = $conn->query("SELECT MAX(recorded_at) as last_updated FROM currency_rates_history");
    if ($res && $row = $res->fetch_assoc()) {
        $last_update = strtotime($row['last_updated']);
        $now = time();
        if ($now - $last_update > 1800) { // 30 dakika geçtiyse
    ...
    <table border="1" class="currency-table">
        <tr>
            <th>Döviz Kodu</th>
            <th>Adı</th>
            <th>Alış Fiyatı</th>
        ...
        $updateQuery = $conn->query("SELECT MAX(recorded_at) AS last_updated FROM currency_rates_history");
        if ($updateQuery && $updateRow = $updateQuery->fetch_assoc()) {
            $formattedDate = date("d.m.Y H:i", strtotime($updateRow['last_updated']));
            echo "<caption style='caption-side: bottom; text-align: left; padding-top: 8px; font-style: italic; margin-left: 9px'>
